...
int main() { int i, j, k, m; int min, max; int pos[2][2];

// Read order of square matrix
...
for (i = 0; i < m; i++) {
    ...
    // Step 7.1: Assume first element is minimum
    min = a[i][0];
    pos[0][0] = i;
    pos[0][1] = 0;

    // Step 7.2: Find minimum in row
    for (j = 1; j < m; j++) {
        if (a[i][j] < min) {
            min = a[i][j];
            pos[0][0] = i;
    ...
    // Step 7.3: Column index of row minimum
    j = pos[0][1];

    // Step 7.4: Assume first element of column is maximum
    max = a[0][j];
    pos[1][0] = 0;
    pos[1][1] = j;

    // Step 7.5: Find maximum in column
    for (k = 1; k < m; k++) {
        if (a[k][j] > max) {
            max = a[k][j];
            pos[1][0] = k;
...
# Output:
<img width="577" height="692" alt="image" src="https://github.com/user-attachments/assets/37a1b827-6e7e-4365-8af7-b9acd8a456ed" />
...
int main() { char s[100], d[100]; int i, len = 0, j = 0; printf("Enter a string: "); scanf("%[^\n]s", s); while (s[len] != '\0') { len++; } for (i = len - 1; i >= 0; i--) { d[j] = s[i]; j++; } d[j] = '\0'; printf("Reversed string: %s", d);

return 0;

# Output:
<img width="1550" height="644" alt="image" src="https://github.com/user-attachments/assets/c6cbbbbc-579c-4a0e-966f-2da771aeaaf9" />
...
int main() { char s[100]; int visited[256] = {0}; int i, j, n, count; printf("Enter a string: "); scanf("%[^\n]", s); n = strlen(s); for (i = 0; i < n; i++) {
...
return 0;
# Output:
<img width="1659" height="650" alt="image" src="https://github.com/user-attachments/assets/6d5a2e10-8e5b-47d7-9b61-1c6eb3a40844" />
...
int main() { char str[200]; char words[50][50]; int i = 0, j = 0, k = 0; int wordCount = 0; int isDuplicate;
...
// Step 6: Find and mark duplicate words
